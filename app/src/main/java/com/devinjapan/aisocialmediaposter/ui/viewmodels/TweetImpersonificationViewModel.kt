...
import com.devinjapan.aisocialmediaposter.ui.state.TweetImpersonificationScreenState
...
class TweetImpersonificationViewModel(
    ...
    var state by mutableStateOf(TweetImpersonificationScreenState())
        private set
    ...
    fun generateDescription() {
        ...
        viewModelScope.launch {
            ...
            when (
                val result =
                    textCompletionRepository.getTweetImpersonificationFromTextCompletionAPI(
                ...
                is Resource.Error<*> -> {
                    ...
                    if (message != null) {
                        state = state.copy(
                            textCompletion = null,
                            isLoading = false,
                            error = TweetImpersonificationScreenState.ErrorInfo(
                                message,
                                result.exception
                            )
                        )
    ...
    fun removeTag(tag: String) {
        ...
        if (state.loadedTags.size <= MAX_KEYWORDS) {
            state = state.copy(
                keywordError = TweetImpersonificationScreenState.ValidationError.NONE
            )
        }
    }
    ...
    fun getSelectedTone() {
        viewModelScope.launch {
            ...
            state = state.copy(
                selectedPersonality = selectedTone,
                isLoading = false
            )
        }
    }
    ...
    fun validateKeyword(keyword: String) {
        val keywordErrorState = if (state.loadedTags.size >= MAX_KEYWORDS) {
            TweetImpersonificationScreenState.ValidationError.TOO_MANY_KEYWORDS
        } else if (keyword.length > MAX_KEYWORD_LENGTH) {
            TweetImpersonificationScreenState.ValidationError.TOO_LONG
        } else {
            TweetImpersonificationScreenState.ValidationError.NONE
        }
        state = state.copy(
